...
# 1. Собираем backend
# 2. Деплоим backend на сервер
# 3. Ждем, пока backend будет готов
# 4. Собираем frontend с переменной окружения для API
# 5. Деплоим frontend
...
# Шаг 1: Собираем и пушим backend
echo "🔨 Шаг 1: Собираю backend Docker образ..."
...
# Шаг 2: Деплоим backend на сервер
echo "🚀 Шаг 2: Деплою backend на сервер..."
...
# Шаг 3: Собираем frontend с переменной окружения для API
echo "🔨 Шаг 3: Собираю frontend Docker образ..."
...
# Шаг 4: Деплоим frontend на сервер
echo "🚀 Шаг 4: Деплою frontend на сервер..."
ssh -i ${SSH_KEY_PATH} ${SERVER_USER}@${SERVER_IP} << 'EOF'
    cd /root
